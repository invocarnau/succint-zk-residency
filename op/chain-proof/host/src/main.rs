...
async fn main() -> eyre::Result<()> {
    ...
    let proof_consensus: SP1ProofWithPublicValues = SP1ProofWithPublicValues::load(
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join(format!("../../proof/chain{}/game_{}.bin", provider_config.chain_id, args.game_index))
    ...
    let consensus_commit = proof_consensus.public_values.clone().read::<OPConsensusCommit>();
